...
var log = debug("urbit-hall-client:debug")
...
async function subscribe(session, wire, callback, from, to) {
  ...
  var decoratedCallback = function(wire, data) {
    log("messages", wire, data)

    if (data == null) {
      // got %quit
      log("got %quit for: " + wire)
...
function processSpeech(m, speech, serial) {
  ...
  if (type == 'lin') {
    ...
    if (speech.fat.tac.text) {
      ...
      message["attachmentLabel"] = speech.fat.tac.name.nom
    ...
    log("Unhandled speech: %" + type, speech)
...
async function sendMessage(session, text, audience) {
  ...
  if (!res) {
    log('Send Error - An error occured while sending the message', payload)
    return false
  }
